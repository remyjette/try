...
type test_result = Success | Failure of string
let test_results : (descr * test_result) list ref = ref []
    ...
    if should_run then begin
      ...
      try
        ...
        if failed then begin
          incr tests_failed;
          test_results := (descr, Failure (descr ^ " is false.\n" ^ (string_of_module_descr ()))) :: !test_results;
          eprintf_or_delay "%s is false.\n%s\n%!" descr
            (string_of_module_descr ())
        end
        else test_results := (descr, Success) :: !test_results
      with exn ->
        print_time_taken ();
        let backtrace = backtrace_indented ~by:2 in
        incr tests_failed;
        let exn_str = Printexc.to_string exn in
        test_results := (descr, Failure (descr ^ " threw " ^ exn_str ^ ".\n" ^ backtrace ^ (string_of_module_descr ()))) :: !test_results;
